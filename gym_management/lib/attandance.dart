...
class _AttandanceState extends State<Attandance> with TickerProviderStateMixin{
  ...
  void initState() {
   ...
   // checkForValue();
   getList();
   attandanceDateShow =  DateFormat("dd").format(attandanceDate) + "  "  +DateFormat("MMMM").format(attandanceDate)+"  "+ DateFormat("y").format(attandanceDate);
  ...
  Widget build(BuildContext context) {
    ...
    return Container(width: unitwidth*100,height: unitheight*90, color: Colors.grey[900],
    child: ListView(
...
Row(children: [
  ...
  AutoSizeText("Attandance Date",
      style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.white,
        fontSize: unitwidth*3)),textScaleFactor: textscale,
...
child: MaterialButton(padding: EdgeInsets.zero,onPressed: (){showDatePicker(context: context, initialDate: DateTime.now(), firstDate: DateTime(DateTime.now().year), lastDate:DateTime(DateTime.now().year+1)).then((value) => {
       ...
       if(value != null){
        ...
        attandanceDateShow =""+ DateFormat("dd").format(value) + "  "  +DateFormat("MMMM").format(value)+"  "+ DateFormat("y").format(value);
       }
       }),
       checkForLength()
      ...
      textStyle: TextStyle(
       color: Colors.black,
        fontSize: unitwidth*2.5 , fontWeight: FontWeight.w600)),textScaleFactor: textscale)),
),
  ...
  Text(attandanceDateShow,
      ...
      textStyle: TextStyle(
       color: Colors.white,
        fontSize: unitwidth*3 , fontWeight: FontWeight.w500)),textScaleFactor: textscale
    ...
    if(tabIndex == 0 )
    Container(width: unitwidth*100,height: unitheight*5,alignment: Alignment.center, child: Row(
    ...
    if(global.addUserTopresentList != null && global.addUserTopresentList!.isNotEmpty){
     ...
     },child:  Container(width: unitwidth*30,height: unitheight*8,alignment: Alignment.center ,decoration: BoxDecoration(
                                    color: Colors.blue, borderRadius: BorderRadius.all(Radius.circular(unitwidth*2))
                                    
                                  ), child: Text("Submit" ,  style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.white,
        fontSize: unitwidth*4,fontWeight: FontWeight.w600)),textScaleFactor: textscale) ,),),

    MaterialButton(onPressed: (){
      finishDayDialog(context);
      },child:  Container(width: unitwidth*30,height: unitheight*6, alignment: Alignment.center,decoration: BoxDecoration(
                                    color: Colors.blue, borderRadius: BorderRadius.all(Radius.circular(unitwidth*2))
                                    
                                  ),child:  Text("Finish Days" ,  style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.white,
        fontSize: unitwidth*4,fontWeight: FontWeight.w600 )),textScaleFactor: textscale),),)
    ],),)else
    ...
    Container(width: unitwidth*100,height: unitheight*5,alignment: Alignment.center, child: Row(
      ...
      MaterialButton(onPressed: (){
      if(tabIndex == 1){
         if(global.deleteUserPresentList != null && global.deleteUserPresentList!.isNotEmpty){
          //global.presentList!.addAll(global.addUserTopresentList!);
           deleteAttandance(global.deleteUserPresentList! , attandance.present);
          //global.addUserTopresentList = [];
      ...
      //  }
       setState(() {
         ...
         if(global.deleteUserAbsentList != null && global.deleteUserAbsentList!.isNotEmpty){
          //global.presentList!.addAll(global.addUserTopresentList!);
           deleteAttandance(global.deleteUserAbsentList! , attandance.absent);
          //global.addUserTopresentList = [];
...
Text("Please pick a date" , style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.red,
        fontSize: unitwidth*2.5 , fontWeight: FontWeight.w600)),textScaleFactor: textscale),
      ],),

Container(width: unitwidth*100,height: unitheight*5,margin: EdgeInsets.symmetric(vertical: unitwidth*3),  child: 
TabBar(tabs: [
  Tab(child: Text("Active User ($activeUserlength)" ,  style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.white,
        fontSize: unitwidth*3)),textScaleFactor: textscale),),
Tab(child: Text("Present User ($presentUserlength)" ,  style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.white,
        fontSize: unitwidth*3)),textScaleFactor: textscale),),
        Tab(child: Text("Absent User ($absentUserlength)" ,  style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.white,
        fontSize: unitwidth*3)),textScaleFactor: textscale),),
] , controller: _tabController, onTap: _onItemTapped , indicatorColor: Colors.blue ,indicatorWeight: unitwidth*0.5, ),),

Container(width: unitwidth*100,height: unitheight*60,color: Colors.grey[900],child: ListView( shrinkWrap: true ,children: [
FutureBuilder(future: getUserList(),  builder: (context, snapshot) {
  if(!snapshot.hasData){
    ...
    return       Column(children: [
           ...
           Column(children: [
                 Container(width: unitwidth*100,height: unitwidth*13,decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.all(Radius.circular(unitwidth*2))), margin:EdgeInsets.symmetric(horizontal: unitwidth*7, vertical: unitwidth*2),child: TextFormField(
    ...
    hintStyle: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              ...
                                              fontSize: unitwidth *
                                                  1 *
                                                  3)),
                                                  prefixIcon:  Icon(Icons.search ,  color: Colors.blue,size:unitwidth* 4),
  ...
  keyboardType: TextInputType.text,
        ...
        //  for(int i =0 ; i < searchedUser.length ; i++ )
          ...
          if(tabIndex == 0)
          for(int index=0 ;index< searchedUser.length;index++)
           if(searchedUser.isNotEmpty &&  searchedUser.isNotEmpty && checkForUserPresent(searchedUser[index].registrationNo!))
               AttandanceList(name: searchedUser[index].name!, registrationNo: searchedUser[index].registrationNo.toString() , a: attandance.active,notifyParent: refreshData, id: "1", time: "" , select: isSelect,)
             else
    ...
    if(tabIndex == 1)
          ...
          AttandanceList(name: presentUser[i].user!.name!, registrationNo: presentUser[i].user!.registrationNo!.toString() ,a: attandance.present,notifyParent: refreshData , id:  presentUser[i].id.toString() , time: (presentUser[i].attandanceTime != null )?presentUser[i].attandanceTime!:AttandanceTime.morning.name, select: isSelect),

    if(tabIndex == 2)
   
         for(int i =0 ; i < absentUser.length ; i++ )
          //if(checkForUserPresent())
          AttandanceList(name: absentUser[i].user!.name!, registrationNo: absentUser[i].user!.registrationNo.toString() , a: attandance.absent,notifyParent: refreshData ,  id: absentUser[i].id.toString() , time: (absentUser[i].attandanceTime != null)?absentUser[i].attandanceTime!:AttandanceTime.morning.name, select: isSelect)
  ...
  if(value.isEmpty){
   list = global.userList!;
    ...
    //    }
    // }
    list = global.userList!.where((element) =>element.name!.toLowerCase().contains(value.toLowerCase())  ).toList();
  }
  }else if(tabIndex == 1){
  if(value.isEmpty){
   presentList = global.presentList;
  }else{
    presentList = global.presentList.where((element) =>element.user!.name!.toLowerCase().contains(value.toLowerCase()) ).toList();
  }
  }else{
  if(value.isEmpty){
   absentL = global.absentList;
  }else{
    absentL = global.absentList.where((element) =>element.user!.name!.toLowerCase().contains(value.toLowerCase()) || element.user!.registrationNo.toString().contains(value) ).toList();
  }
  }
  ...
  submitAttandance(List<int> registationNo)async{
    ...
    if(!waitForSubmitResponse){
      data = await Services().submitAttandance(registationNo , attandanceDate.toString() ,attandanceTime.name );
    }
   ...
   if(data.toString() == "true"){
    ...
    waitForSubmitResponse = false;
   Fluttertoast.showToast(msg: "Please submit again " , toastLength: Toast.LENGTH_LONG);
   }
  }
  ...
  deleteAttandance(List<int> list , attandance value)async{
    ...
    if(!waitFordeleteList){
      data = await Services().deleteAttandanceList(list ,attandanceDate.toString(),  value);
    }
  ...
  getPresentUserList()async{
    List<AttandanceModel> attandance = await Services().getUserAttandanceByStatus(attandanceStatus.present , attandanceDate.toString());
    print("Attandance present "+ attandance.length.toString());
    return attandance;
  }
  getAbsentUserList()async{
    List<AttandanceModel> attandance = await Services().getUserAttandanceByStatus(attandanceStatus.absent , attandanceDate.toString());
  ...
  list(double width , double height , String name){
    ...
    return Container(width: width*100,height: height*7,color: Colors.blue.withOpacity(0.1), margin: EdgeInsets.symmetric(horizontal: width*5 , vertical: height*1),child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround,crossAxisAlignment: CrossAxisAlignment.center,  children: [
      Container(width: width*13, height: width*13, color: Colors.white, child: Icon(Icons.people),),
           ...
           var res;
            if(!waitForFinishResponse){
              waitForFinishResponse = true;
               res = await Services().finishDay(attandanceDate.toString() , attandanceTime.name);
            }
...
class AttandanceList extends StatefulWidget {
  ...
  bool select;
   AttandanceList({super.key , required this.id,required this.name ,required this.registrationNo , required this.a ,required this.time, required this.notifyParent , required this.select});
...
class _AttandanceListState extends State<AttandanceList> {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(width: width*100,height: height*7,decoration: BoxDecoration(color: Colors.white.withOpacity(0.8),borderRadius: BorderRadius.all(Radius.circular(10))), margin: EdgeInsets.symmetric(horizontal: width*5 , vertical: height*1),child: Row(mainAxisAlignment: MainAxisAlignment.spaceAround,crossAxisAlignment: CrossAxisAlignment.center,  children: [
      if(widget.a != attandance.active)
      Container(width: width*13, height: width*13,alignment: Alignment.center ,color: Colors.white.withOpacity(0.1), child:  Text((widget.time.isNotEmpty && widget.time == AttandanceTime.morning.name)?"Morning":"Evening", style: GoogleFonts.montserrat(
      textStyle: TextStyle(fontWeight: FontWeight.w500,
       color: Colors.black.withOpacity(0.9),
        fontSize: width*2.8)),textScaleFactor: textscale,),),
         if(widget.a == attandance.active)
      Container(width: width*13, height: width*13,alignment: Alignment.center ,color: Colors.white.withOpacity(0.1), child:  Icon(Icons.people),),
     Column(mainAxisAlignment: MainAxisAlignment.center,
      children: [
 AutoSizeText(widget.name , style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.black,fontWeight: FontWeight.w600,
        fontSize: width*4)),textScaleFactor: textscale),
        SizedBox(height: height*0.5,),
         Text("Registration no -"+widget.registrationNo  , style: GoogleFonts.montserrat(
      textStyle: TextStyle(fontWeight: FontWeight.w500,
       color: Colors.black.withOpacity(0.6),
        fontSize: width*2.8)),textScaleFactor: textscale,),
     ],),
         if(widget.a == attandance.active)
         Container(width: width*16,height: width*8,   child: Checkbox(activeColor: Colors.black,checkColor: Colors.white, onChanged: ((value) => {
         setState(() {
       select = value!;
       if(value){
        global.addUserTopresentList!.add(int.parse(widget.registrationNo));
                
       }else{
         if(global.addUserTopresentList != null && global.addUserTopresentList!.isNotEmpty){

          if(global.addUserTopresentList!.contains(int.parse(widget.registrationNo))){
            
            global.addUserTopresentList!.remove(int.parse(widget.registrationNo));
       ...
       if(widget.a == attandance.present){
           if(value){
        global.deleteUserPresentList!.add(int.parse(widget.id));
                
       }else{
         if(global.deleteUserPresentList != null && global.deleteUserPresentList!.isNotEmpty){

          if(global.deleteUserPresentList!.contains(int.parse(widget.id))){
            
            global.deleteUserPresentList!.remove(int.parse(widget.id));
          ...
          if(value){
        global.deleteUserAbsentList!.add(int.parse(widget.id));
                
       }else{
         if(global.deleteUserAbsentList != null && global.deleteUserAbsentList!.isNotEmpty){

          if(global.deleteUserAbsentList!.contains(int.parse(widget.id))){
            
            global.deleteUserAbsentList!.remove(int.parse(widget.id));
