...
class _SearchListState extends State<SearchList> {
  ...
  Widget build(BuildContext context) {
    ...
    return MaterialButton( onPressed: () {
      ...
      child: Container(width: unitwidth *100,height: unitheight*11.5 ,decoration: BoxDecoration( color: Colors.grey.shade800,
      borderRadius: BorderRadius.all(Radius.circular(10))) ,margin:EdgeInsets.symmetric(horizontal: unitwidth*2 , vertical: unitheight*1.2) , alignment: Alignment.center, child: Row(
        ...
        children: [
         Container(width: unitwidth*15,height: unitwidth*20.5,alignment: Alignment.center,decoration: BoxDecoration(borderRadius: BorderRadius.all(Radius.circular(unitwidth*7)),color: Colors.black.withOpacity(0.1))  ,child:Icon(Icons.people , color: Colors.white) ,),
         Container(width: unitwidth*40,  height: unitwidth*20.5,alignment: Alignment.center, child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
             AutoSizeText(widget.user.name.toString(),style: GoogleFonts.montserrat(
                                            textStyle: TextStyle(
                                                color: Colors.white,fontWeight: FontWeight.w600,
                                                fontSize: unitwidth *
                                                    1 *
                                                    3.5)),textScaleFactor: textscale),
                                                    SizedBox(height: unitwidth*1,),
             AutoSizeText("Registration No - ${widget.user.registrationNo}",style: GoogleFonts.montserrat(
                                            textStyle: TextStyle(
                                                ...
                                                fontSize: unitwidth *
                                                    ...
                                                    SizedBox(height: unitwidth*1,),
             AutoSizeText("Phone No - ${widget.user.phoneNo}",style: GoogleFonts.montserrat(
                                            textStyle: TextStyle(
                                                color: Colors.white.withOpacity(0.6),fontWeight: FontWeight.w600,
                                                fontSize: unitwidth *
                                                    1 *
                                                    ...
                                                    SizedBox(height: unitwidth*1,),
             AutoSizeText("Address - ${widget.user.address}",textAlign: TextAlign.center, style: GoogleFonts.montserrat(
                                            textStyle: TextStyle(
                                                color: Colors.white.withOpacity(0.6),fontWeight: FontWeight.w600,overflow: TextOverflow.fade,
                                                fontSize: unitwidth *
                                                    1 *
                                                    2)),textScaleFactor: textscale,),
          ],
         ),),
         ...
         Column(children: [
           AutoSizeText(snapshot.data["days"],style: GoogleFonts.montserrat(
                   ...
                   AutoSizeText(snapshot.data["des"],style: GoogleFonts.montserrat(
                                            textStyle: TextStyle(
                                                color: Colors.red.withOpacity(0.6),fontWeight: FontWeight.w900,
                                                fontSize:(snapshot.data["sI"] != "day") ? unitwidth *
                                                    2
  ...
  if(remainingDays == 0){
    ...
    return await {"days":remainingDays.toString() , "size":9.2 , "des":s ,"sI":sizeIdentifier};
  }
  ...
  getInfoDesign(double width , double height , String title , String value){
    return Container(width: width * 40, height: height*5,margin: EdgeInsets.symmetric(vertical: 5) ,alignment: Alignment.center,child: Column(
      children: [
        Text("$title:",style: GoogleFonts.alegreyaSc(textStyle: TextStyle( color: Colors.blue.shade900)),)  , Text("$value")
      ],
    ),);
  }
}