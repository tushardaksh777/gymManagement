...
class _EditUserState extends State<EditUser> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: Text("Add Member")), 
...
Text("Registration No" , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              color: Colors.black,
                                              fontSize: unitwidth *
                                                  1 *
                                                  4)),),
                                                   SizedBox(height: unitheight*2,),
        Container(width: unitwidth*100,height: unitwidth*13, margin:EdgeInsets.symmetric(horizontal: unitwidth*7, vertical: unitwidth*2),child: TextFormField(
    ...
    hintStyle: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              ...
                                              fontSize: unitwidth *
                                                  1 *
                                                  3)),
                                                  prefixIcon:  Icon(Icons.app_registration ,  color: Colors.blue,size:unitwidth* 4),
                                                  
  ),controller: registrationNoController,autovalidateMode: AutovalidateMode.always, validator: (value) {
...
Text("Name" , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              ...
                                              fontSize: unitwidth *
                                                  1 *
                                                  4)),),
Container(width: unitwidth*100,height: unitwidth*15, margin:EdgeInsets.symmetric(horizontal: unitwidth*7, vertical: unitwidth*2),child: TextFormField(
    ...
    hintStyle: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              ...
                                              fontSize: unitwidth *
                                                  1 *
                                                  3)),
                                                  prefixIcon:  Icon(Icons.person , color: Colors.blue,size:unitwidth* 4),
                                                  
  ),controller: nameController,autovalidateMode: AutovalidateMode.always, validator: (value) {
    if(value == null|| value.isEmpty){
...
SizedBox(height: unitheight*2,),
Text("Father's Name" , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              color: Colors.black,
                                              fontSize: unitwidth *
                                                  1 *
...
Container(width: unitwidth*100,height: unitwidth*15, margin:EdgeInsets.symmetric(horizontal: unitwidth*7, vertical: unitwidth*2),child: TextFormField(
  ...
  ),controller: fatherNameController,autovalidateMode: AutovalidateMode.always, validator: (value) {
    if(value == null|| value.isEmpty){
...
SizedBox(height: unitheight*2,),
Text("Husband's Name" , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              color: Colors.black,
                                              fontSize: unitwidth *
                                                  1 *
...
Container(width: unitwidth*100,height: unitwidth*15, margin:EdgeInsets.symmetric(horizontal: unitwidth*7, vertical: unitwidth*2),child: TextFormField(
  ...
  ),controller: husbandNameController,autovalidateMode: AutovalidateMode.always, validator: (value) {
    if(value == null|| value.isEmpty){
...
SizedBox(height: unitheight*2,),
Text("Phone No." , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              color: Colors.black,
                                              fontSize: unitwidth *
                                                  1 *
    ...
    hintStyle: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              ...
                                              fontSize: unitwidth *
                                                  1 *
                                                  3)),
                                                  prefixIcon:  Icon(FontAwesomeIcons.addressBook ,  color: Colors.blue,size:unitwidth* 4),
...
Container(width: unitwidth*18,height: unitheight*3.5,alignment: Alignment.center,decoration: BoxDecoration(color: Colors.blue.withOpacity(0.3) , borderRadius: BorderRadius.all(Radius.circular(unitwidth*1.5))),
child: MaterialButton(padding: EdgeInsets.zero,onPressed: (){showDatePicker(context: context, initialDate: DateTime.now(), firstDate: DateTime(DateTime.now().year), lastDate: DateTime(DateTime.now().year  ,DateTime.now().month +1   )).then((value) => {
       setState(() {
        joiningDate = value;
        joiningDateShow =""+ DateFormat("dd").format(joiningDate!) + "  "  +DateFormat("MMMM").format(joiningDate!)+"  "+ DateFormat("y").format(joiningDate!);
       })
  ...
  SizedBox(width: unitwidth*10,),
  Text(joiningDateShow,
      style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.black,
...
Column(
  children: [
    Divider(thickness: 10, height: unitheight*5,color: Colors.grey[300],),
    Text("Plan Details" , style: GoogleFonts.montserrat(textStyle: TextStyle(fontSize: unitwidth*6)),),
Container(width: unitwidth*100,height: unitwidth*13, margin:EdgeInsets.symmetric(horizontal: unitwidth*7, vertical: unitwidth*2), child: Row(
  mainAxisAlignment: MainAxisAlignment.spaceEvenly,
children: [
...
Row(children: [
  ...
  ),controller: registration_Fees_Controller,autovalidateMode: AutovalidateMode.always, validator: (value) {
    if(value == null|| value.isEmpty){
...
Container(width: unitwidth*80,height: unitwidth*15, margin:EdgeInsets.symmetric(horizontal: unitwidth*7, vertical: unitwidth*2),child: TextFormField(
      textInputAction: TextInputAction.done,
      style: GoogleFonts.montserrat(
      textStyle: TextStyle(
       color: Colors.black,
    ...
    hintStyle: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              ...
                                              fontSize: unitwidth *
                                                  1 *
                                                  3)),
                                                  prefixIcon:  Icon(Icons.note ,  color: Colors.blue,size:unitwidth* 4),
...
SizedBox(height: unitheight*5,)
  ...
  setexistingData(){
    ...
    nameController.text = widget.user.name!;
     fatherNameController.text = (widget.user.fatherName != null)?widget.user.fatherName!:"";
     phoneNoController.text = widget.user.phoneNo!;
     addressController.text = widget.user.address!;
     husbandNameController.text = ((widget.user.husbandName != null)?widget.user.husbandName!:"");
     //registration_Fees_Controller.text = widget.user.registraionFee!.toString();
     _activeMode = (widget.user.status == "active")?activeMode.active:activeMode.inactive;
  ...
  String registrationNo = (registrationNoController.text.isEmpty)? widget.user.registrationNo.toString(): registrationNoController.text;
  String Name = (nameController.text).isEmpty?widget.user.name!:nameController.text;
  String fatherName = "";
  ...
  String phoneNo = (phoneNoController.text.isEmpty)?widget.user.phoneNo!:phoneNoController.text;
  String Address = (addressController.text.isEmpty)?widget.user.address!:addressController.text;
  String husbandName ="";
 ...
 // int dif = joiningDate!.difference(DateTime.parse(widget.user.joiningDate!)).inDays;
  ...
  String joinDate = DateTime(joiningDate!.year , joiningDate!.month , joiningDate!.day).toString();
      ...
      actions: [
        MaterialButton(
          onPressed: () async {
            if(!response){
              response = true;
            var u = await Services().editUser(user);    //await Services().addUser(user);
