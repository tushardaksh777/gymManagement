...
class _HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(color: Colors.grey[900], child: RefreshIndicator(
              onRefresh: reload,
              child: ListView(
                shrinkWrap: true,
                children: [
...
Row(children: [
  Radio(value: value.active, groupValue: _value, onChanged: (onChanged){
setState(() {
  //print("Radio 1" + onChanged!.toString());
  _value = onChanged!;
});
} , activeColor: Colors.white),
Text("Active ($activeUser)" , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              color: Colors.white,
                                              fontSize: unitwidth *
                                                  1 *
...
Row(children: [
  Radio(value: value.inactive, groupValue: _value, onChanged: (onChanged){
setState(() {
  //print("Radio 1" + onChanged!.toString());
  _value = onChanged!;
});
} , activeColor: Colors.white),
Text("Inactive ($inactiveUser)" , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              color: Colors.white,
                                              fontSize: unitwidth *
                                                  1 *
...
Row(children: [
  Radio(value: value.all, groupValue: _value, onChanged: (onChanged){
setState(() {
 //print("Radio 1" + onChanged!.toString());
  _value = onChanged!;
});
} , activeColor: Colors.white),
Text("All User ($allUser)" , style: GoogleFonts.montserrat(
                                          textStyle: TextStyle(
                                              color: Colors.white,
                                              fontSize: unitwidth *
                                                  1 *
...
Divider(height: unitwidth*2 , thickness: unitwidth*1,),
if(_value == value.active)
Column(children: [
Container(width: unitwidth*100,height: unitheight*5,
            child: TabBar(
              automaticIndicatorColorAdjustment: false,
              tabs: [
  Tab(child: Text("Remaining Days ($remainingUser)" , style:  GoogleFonts.montserrat(
                                            textStyle: TextStyle(
                                                color: Colors.white.withOpacity(0.7),fontWeight: FontWeight.w600,
                                                fontSize: unitwidth *
                                                    1 *
                                                    3.5)),),),
Tab(child:Text("OverDue Days ($overdueUser)" , style:  GoogleFonts.montserrat(
                                            textStyle: TextStyle(
                                                color: Colors.white.withOpacity(0.7),fontWeight: FontWeight.w600,
                                                fontSize: unitwidth *
                                                    1 *
            ...
            FutureBuilder(future: getUser(),  builder:  (BuildContext context, AsyncSnapshot snapshot) {
              if(!snapshot.hasData){
