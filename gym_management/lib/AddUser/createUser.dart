...
),),Container(width: unitwidth*100,height: unitwidth*13, margin:EdgeInsets.symmetric(horizontal: unitwidth*6, vertical: unitwidth*2),child: TextFormField(
      textInputAction: TextInputAction.next,
      maxLength: 35,
      style: GoogleFonts.montserrat(
      textStyle: TextStyle(
...
child: MaterialButton(padding: EdgeInsets.zero,onPressed: (){showDatePicker(context: context, initialDate: DateTime.now(), firstDate: DateTime(DateTime.now().year -2), lastDate:DateTime(DateTime.now().year+1)).then((value) => {
       setState(() {
        //print("Date "+value.toString());
        joiningDate = value;
        joiningDateShow =""+ DateFormat("dd").format(value!) + "  "  +DateFormat("MMMM").format(value)+"  "+ DateFormat("y").format(value);
       })
...
SizedBox(height: unitheight*5,)
    ...
    AlertDialog alertDialog = AlertDialog(
      ...
      actions: [
        MaterialButton(
          onPressed: () async {
            if(!response){
            var u = await Services().addUser(user);     //await Services().addUser(user);
  ...
  String fatherName = (fatherNameController.text.isEmpty)?"":fatherNameController.text;
  String phoneNo = phoneNoController.text;
  String Address = addressController.text;
  String joinDate = (joiningDate == null)?"":joiningDate.toString();
  String husbandName = (husbandNameController.text.isEmpty)?"":husbandNameController.text;
  ...
  bool validate = false;
  if(registrationNoController.text.isNotEmpty && nameController.text.isNotEmpty && phoneNoController.text.isNotEmpty && addressController.text.isNotEmpty){
    validate = true;
  }
     ...
     if(validate)
      {
        ...
        userModel.joiningDate = (joinDate.isEmpty)? DateTime.now().toString():joinDate;
        // if(_activeMode == activeMode.active){
